...
class minimax:
    ...
    def expanded_nodes_no_pruning(board):
        if board.children is None:
            return 0
        else:
            count = 1
            for child in board.children:
                count += minimax.expanded_nodes_no_pruning(child)
            return count

    def expanded_nodes_pruning(board):
        if board.children is None:
            return 0
        else:
            count = 1
            for child in board.children:
                if not child.pruned:
                    count += minimax.expanded_nodes_pruning(child)
            return count

    def minimax_decision(board,print_tree):
        (child, utility) = minimax.max_value(board, board.depth)
        board.utility = utility
        print("Expanded nodes for minimax: ", minimax.expanded_nodes_no_pruning(board))
        if print_tree:
    ...
    def minimax_decision_pruning(board,print_tree):
        (child, utility) = minimax.max_value_pruning(board, board.depth, float('-inf'), float('inf'))
        board.utility = utility
        expanded_nodes=minimax.expanded_nodes_pruning(board)
        print("Expanded nodes for minimax with pruning: ",expanded_nodes)
        if print_tree:
